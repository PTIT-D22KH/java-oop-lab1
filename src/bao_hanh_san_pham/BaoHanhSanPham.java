...
//        Scanner input = new Scanner(new File("src/file_container/bao_hanh_san_pham/MUAHANG.in"));
        ...
        for (int i = 0; i < m; i++) {
            customers.add(new Customer(input.nextLine(), input.nextLine(), input.nextLine(), Integer.parseInt(input.nextLine()), input.nextLine()));
        }
        ...
        for (Customer x : customers) {
            for (Product y : products) {
                ...
                if (x.getProductId().equals(y.getProductId())) {
                    x.calWarrantyDate(y.getWarrantyTime());
                    x.calTotalCost(y.getPrice());
                    break;
                }
            }
        }
